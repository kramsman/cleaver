...
#   Renamed imported code sheets to first, middle, last from Precounty, Modicode, Remcode
#   Changed get_addr_concentration to return a tuple with (desc,remove reason) and removed flag
...
# V15.1 redo split to limit limit into routine chunk.  Filenames had to be kept in parallel and didn't chunk with no
# splitfield
...
# TODO: add flag to tabluates for clean vs all
...
INITIAL_CAMPAIGN_DIR = os.path.expanduser(r"~/Dropbox/Postcard Files/TestInputFiles/TestCampaigns")
...
def create_import_code_from_sheet(setup_wb, sheet_with_python_code, output_file):
    ...
    with open(output_file, "w") as new_code_text:
...
def create_dicts():
    ...
    ROV.dict_statecounty_to_alt_formats = counties_to_xlsx('zip-codes-database-DELUXE-BUSINESS.csv',
...
def display_imported_code(sheet_name, py_file_name):
    ...
    with open(py_file_name, "r") as myfile:  # this copies in the code but does not execute it
...
def loop_through_format_files(filelist_wks):
    ...
    for fn, format_flag, combine_flag, update_fn, update_fields, *_ in islice(filelist_wks, 1, None):
        # islice starts at row index 1 not 0 to skip header; *_ discards unused cols (notes, etc)
        if str(format_flag.value).strip().lower() == "x":

            ip = create_one_format_file(fn.value,
                                        ROV.rawdata_path,
                                        ROV.format_path,
    ...
    if cumulative_missing_counties_list:
        pymsgbox.alert("The following counties are not in the lookup file, written to 'missing_counties.csv':\n\n" +
                       ...
                       "Alert")
        with open(missing_counties_file, mode='wt', encoding='utf-8') as myfile:
            myfile.write('\n'.join(cumulative_missing_counties_list))
...
def create_one_format_file(fn, input_path, op_path, missing_counties_list_all_formatfiles):
    """
    ...
    ## This is new in V15.0 - rename fields with prefix of '_'
...
def main_program():
    ...
    choice = pymsgbox.confirm("What do you want to do?",
                              'Choose Action',
                              ['Format', 'Combine', 'Split', 'Update Zip File', 'Exit'])
    ...
    if choice == 'Format':
        ...
        pymsgbox.alert("Ran format", "Alert")
        ...
        pymsgbox.alert("Ran combine", "Alert")
        ...
        pymsgbox.alert("Ran split", "Alert")
